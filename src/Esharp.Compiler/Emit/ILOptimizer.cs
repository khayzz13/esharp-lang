...
{
    /// <summary>
    /// Process-wide differential-testing knob: when false, ShortenOpcodes is an
    /// identity pass, so every method body keeps its long-form opcodes. The compacted
    /// and long-form assemblies must behave identically — the fuzz harness emits both
    /// and compares. Production never touches this; it defaults to enabled.
    /// </summary>
    public static bool ShortenOpcodesEnabled { get; set; } = true;
    ...
    public static void ShortenOpcodes(MethodBody body)
    {
        ...
        for (var i = 0; i < instructions.Count; i++)
        {
            ...
            // ParameterDefinition.Index is 0-based from the param list; instance methods
            // have an implicit 'this' at arg 0, so the actual IL arg index = Index + 1.
            else if (instr.OpCode == OpCodes.Ldarg && instr.Operand is ParameterDefinition ldargParam)
            {
                var ilIdx = ldargParam.Index + (body.Method.HasThis ? 1 : 0);
                var replacement = ilIdx switch
                {
                    0 => il.Create(OpCodes.Ldarg_0),
                    1 => il.Create(OpCodes.Ldarg_1),
                    2 => il.Create(OpCodes.Ldarg_2),
                    3 => il.Create(OpCodes.Ldarg_3),
                    _ => ilIdx < 256 ? il.Create(OpCodes.Ldarg_S, ldargParam) : null,
                };
                if (replacement is not null)
                    Replace(il, instr, replacement);
            }
